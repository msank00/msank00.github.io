...
It is only reasonable to assume that if we design a different and more powerful architecture $\mathcal{F}'$ we should arrive at a better outcome. In other words, we would expect that $f^{∗}_{\mathcal F'}$ is **better** than $f^{∗}_{\mathcal F}$. However, if 
