...
- :star: Go to the code-server release [page](https://github.com/cdr/code-server/releases) and **download the latest release always**. They are updating frequently.
...
ssh -R <domain_name>:80:localhost:56790 serveo.net

# go to: https://<domain_name>.serveo.net
```
...
Installing `jupyter`, `tmux` and `code-server` can be done quickly using this [gist](https://gist.github.com/msank00/4fd6ba42c947b93767a968a0c66fabdc).
...
- Run `code-server` in colab
- Run `vscode` in local machine and `ssh` to colab using `remote explorer` plugin. *BEST APPROACH*
